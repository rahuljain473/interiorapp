...
const transporter = nodemailer.createTransport({
    service : 'outlook',
    auth : {
        user : 'HyperMart2023@outlook.com',
        pass : 'Hyperm@rt2023'
    }
  })
...
app.post("/api/register",async (req,res)=>
{
    ...
    {
        const mailOptions = {
            from: 'HyperMart2023@outlook.com',
            to: req.body.username,
            subject: 'Account Activation Mail from HyperMart.com',
...
app.post("/api/login", async (req,res)=>
{
    ...
    else
    {
        ...
        var compareResult = bcrypt.compareSync(req.body.pass, hash);
...
app.get("/api/searchmember", async (req,res)=>
{
    ...
    var result = await registerModel.find({username:req.query.un});
...
app.put("/api/updatecategory",upload.single('picture'),async (req,res)=>
{
    ...
    {
      if(req.body.oldpicname!="noimage.png")
      {
...
var productSchema = new mongoose.Schema({catid:String, subcatid:String, prodname:String, rate:Number, discount: Number, stock:Number, description:String, featured:String, prodpic:String, addedon:String},{versionKey: false})
...
app.post("/api/addproduct",upload.single('picture'),async (req,res)=>
{
    ...
    var newrecord = new productModel({catid:req.body.catid, subcatid:req.body. subcatid, prodname:req.body.pname, rate:req.body.rate, discount:req.body.discount, stock:req.body.stock, description:req.body.description,featured:req.body.featured, prodpic: picname, addedon:new Date});
...
{
    var result = await productModel.find().sort({"addedon":-1}).limit(3);
    if(result.length===0)
    {
        res.send({statuscode:0})
    }
...
{
    var result = await productModel.find({featured:"yes"}).limit(3);
    if(result.length===0)
    {
        res.send({statuscode:0})
    }
